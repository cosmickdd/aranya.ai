  ...
  const permissions = [
    {
      ...
      desc: "Chat naturally in Hindi, Marathi, or English",
      color: "#10b981",
    },
    {
      icon: <MapPin size={22} color="#3b82f6" />,
      title: "Location Services",
      desc: "Get live local weather and find nearest soil labs",
      color: "#3b82f6",
    },
    {
      icon: <CameraIcon size={22} color="#f59e0b" />,
      title: "Camera & Gallery",
      desc: "Scan and diagnose crop health instantly",
      color: "#f59e0b",
    },
  ];
      ...
      >
        ...
        >
          ...
          <View style={{ alignItems: "center", marginBottom: 26 }}>
            ...
            >
              Allow the following permissions to enable real-time local
  ...
  return parts.map((part, index) => {
    if (index % 2 === 1) {
      return (
        <Text
          key={index}
          style={{ fontWeight: "700", color: isSender ? "#ffffff" : "#1f2937" }}
        >
          {part}
        </Text>
      );
...
export default function Dashboard() {
  ...
  const recorder = useAudioRecorder({
    ...
    numberOfChannels: 1, // Fix for some Android phones that crash on stereo recording
  ...
  const [cameraMode, setCameraMode] = useState<"photo" | "video" | "videonote">(
    "photo",
  );
  ...
  const loadGalleryPhotos = async () => {
    if (Platform.OS === "web" || !MediaLibrary) {
      setGalleryPhotos([
        {
          id: "1",
          uri: "https://images.unsplash.com/photo-1574323347407-f5e1ad6d020b?w=200&q=80",
        },
        {
          id: "2",
          uri: "https://images.unsplash.com/photo-1536630590251-40439a0f675f?w=200&q=80",
        },
        {
          id: "3",
          uri: "https://images.unsplash.com/photo-1551893086-c02cbf3a9a88?w=200&q=80",
        },
        {
          id: "4",
          uri: "https://images.unsplash.com/photo-1600697395593-e9dc66797e43?w=200&q=80",
        },
      ]);
      return;
    }
  ...
  useEffect(() => {
    if (voiceState === "speaking") {
      ...
      ripple2.value = withDelay(
        1000,
        withRepeat(withTiming(1.6, { duration: 2000 }), -1, false),
      );
    } else {
      ripple1.value = 1;
      ripple2.value = 1;
  ...
  const rippleStyle1 = useAnimatedStyle(() => {
    const scale =
      voiceState === "listening" ? volumeScale.value : ripple1.value;
    const opacity =
  ...
  const rippleStyle2 = useAnimatedStyle(() => {
    const scale =
      voiceState === "listening" ? volumeScale.value * 1.3 : ripple2.value;
    const opacity =
      voiceState === "listening"
  ...
  useEffect(() => {
    AsyncStorage.getItem("aranya_chat_history")
      .then((data) => {
        if (data) {
          try {
  ...
  const playAudio = async (
    audioSource: { base64?: string; uri?: string },
    msgId: string,
  ) => {
    try {
      await stopActiveAudio();
  ...
  const takePicture = async () => {
    if (cameraRef.current) {
      try {
        ...
        console.error(
          "Failed to take picture, falling back to system camera:",
          e,
        );
        const options: ImagePicker.ImagePickerOptions = {
          mediaTypes: ["images"],
  ...
  const shareLocation = async () => {
    ...
    try {
      ...
      if (status !== "granted") {
        Alert.alert(
          "Permission Denied",
          "Location access is required to share coordinates.",
        );
        return;
      }
      const loc = await Location.getCurrentPositionAsync({});
      ...
      const city =
        geocode[0]?.city || geocode[0]?.district || geocode[0]?.region || "";
    ...
    async (
      ...
      try {
        const apiUrl =
          ...
          "https://aranya-ai-6r0j.onrender.com";
        ...
        setMessages((prev) =>
          prev.map((m) => (m.isSender ? { ...m, status: "read" } : m)),
        );
        ...
        if (data.reply) {
          const reply: Message = {
            ...
            voiceDuration: data.audio_base64
              ? Math.ceil(data.reply.length / 15)
              : undefined,
          };
    ...
    try {
      ...
      if (Platform.OS === "web") {
        ...
        formData.append(
          "audio",
          fileBlob,
          Platform.OS === "android" ? "voice.m4a" : "voice.caf",
        );
      }
    ...
    try {
      ...
      if (Platform.OS === "web") {
        ...
        if (audioBlob.size < 1000) {
          ...
          setVoiceState("speaking");
          setVoiceTranscript(
            "Microphone recorded no sound. Please check your mic permissions.",
          );
          clearTimeout(timeoutId);
          setTimeout(() => {
            if (voiceModeRef.current) {
        ...
        console.log("Recorded Audio URI (Native):", fileUri);
        const fileResponse = await fetch(fileUri);
        const fileBlob = await fileResponse.blob();
        formData.append(
          "audio",
      ...
      if (data.audio_base64) {
        ...
        if (sarvamB64) {
          try {
            ...
            console.error("Sarvam playback error:", e);
          }
        } else {
          try {
  ...
  const greetAndStart = async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/chat`, {
        ...
        body: JSON.stringify({
          message:
            "Please give a very short 1-sentence friendly greeting to start our voice call.",
          user_id: "demo_user_123",
          language: i18n.locale,
        }),
      });
    ...
    return (
      <View style={vs.overlay}>
        <ImageBackground
          ...
          style={vs.bg}
          imageStyle={vs.bgImage}
        >
          <View style={vs.topBar}>
            <Pressable
              onPress={() => setIsVoiceMinimized(true)}
              style={vs.iconBtn}
            >
          ...
          >
            <View style={vs.pillContainer}>
              ...
              <Pressable
                style={[
                  vs.pillButton,
                  isListening ? vs.pillButtonActive : null,
                ]}
                onPress={() => {
    ...
    >
      ...
      >
          ...
          <View style={cs.headerLeft}>
            ...
            <View>
              ...
              {isTyping ? (
                <Animated.Text
                  entering={FadeIn}
                  style={[cs.headerStatus, { color: "#fb923c" }]}
                >
                  typing...
                </Animated.Text>
              ) : (
                <Animated.Text entering={FadeIn} style={cs.headerStatus}>
                  Online
                </Animated.Text>
              )}
            </View>
        ...
        {voiceMode && isVoiceMinimized && (
          <Pressable
            style={cs.activeCallBar}
            onPress={() => setIsVoiceMinimized(false)}
          >
            <Text style={cs.activeCallText}>
              Tap to return to call {formatDuration(callDuration)}
            </Text>
          </Pressable>
        )}
        ...
        <View style={cs.chatArea}>
          ...
          >
            <View style={cs.dateBadgeContainer}>
              <View style={cs.dateBadge}>
                <Text style={cs.dateText}>Today</Text>
              </View>
            </View>
            ...
            {messages.map((msg) => (
              <Animated.View
                ...
                style={[
                  cs.messageBubble,
                  msg.isSender ? cs.sentBubble : cs.receivedBubble,
                ]}
              >
                {msg.image_base64 && (
                  <Image
                ...
                {msg.isDocument && (
                  <View
                    style={{
                      flexDirection: "row",
                      alignItems: "center",
                      backgroundColor: msg.isSender ? "#10b981" : "#f3f4f6",
                      borderRadius: 8,
                      padding: 12,
                      marginBottom: 6,
                      borderWidth: 1,
                      borderColor: msg.isSender ? "#059669" : "#e5e7eb",
                    }}
                  >
                    <FileText
                      color={msg.isSender ? "#ffffff" : "#8b5cf6"}
                      size={28}
                    ...
                    <View style={{ flex: 1 }}>
                      <Text
                        style={{
                          fontSize: 14,
                          fontWeight: "600",
                          color: msg.isSender ? "#ffffff" : "#374151",
                        }}
                        numberOfLines={1}
                      >
                      ...
                      <Text
                        style={{
                          fontSize: 11,
                          color: msg.isSender ? "#a7f3d0" : "#6b7280",
                          marginTop: 2,
                        }}
                      >
                        PDF / Document
                ...
                {msg.isVoiceNote ? (
                  <View style={cs.voiceMessageContainer}>
                    <Pressable
                      ...
                      onPress={() =>
                        playAudio(
                          { uri: msg.audio_uri, base64: msg.audio_base64 },
                          msg.id,
                        )
                      }
                    >
                      {playingId === msg.id ? (
                        <Pause
                          color={msg.isSender ? "#ffffff" : "#17c690"}
                          size={16}
                        />
                      ) : (
                        <Play
                          color={msg.isSender ? "#ffffff" : "#4b5563"}
                          size={16}
                          style={{ marginLeft: 2 }}
                        />
                      )}
                      ...
                      <View style={cs.voiceWaveformRow}>
                        {msg.isTranscribing ? (
                          <View style={cs.voiceLoadingRow}>
                            ...
                            <Text
                              style={[
                                cs.voiceTranscribingText,
                                msg.isSender && { color: "#ffedd5" },
                              ]}
                            >
                              Transcribing voice...
                            </Text>
                          ...
                          <View style={cs.voiceWaveform}>
                            {[
                              4, 10, 14, 8, 18, 12, 6, 10, 16, 12, 6, 14, 8, 10,
                              16, 12, 4,
                            ].map((h, i) => (
                              <View
                                key={i}
                  ...
                  >
                    ...
                    >
                      <MapPin
                        color={msg.isSender ? "#ffffff" : "#10b981"}
                        size={20}
                        fill={
                          msg.isSender
                  ...
                  !!msg.text && (
                    <Text style={msg.isSender ? cs.sentText : cs.receivedText}>
                      {renderFormattedText(msg.text, msg.isSender)}
                    </Text>
                  )
                )}
                ...
                {msg.isVoiceNote && !msg.isTranscribing && !!msg.text && (
                  <View
                    style={[
                      cs.transcriptContainer,
                      msg.isSender
                        ? cs.senderTranscriptBg
                        : cs.receivedTranscriptBg,
                    ]}
                  >
                    <Text
                      style={[
                        cs.transcriptHeaderLabel,
                        msg.isSender
                          ? { color: "#ffedd5" }
                          : { color: "#17c690" },
                      ]}
                ...
                {msg.hasCallAction && (
                  <Pressable
                    style={cs.callActionButton}
                    onPress={toggleVoiceMode}
                  >
                    <Phone color="#10b981" size={16} />
                    <Text style={cs.callActionText}>
                      बोलकर बात करें (Voice Chat)
                    </Text>
                  </Pressable>
                )}
                <View style={cs.messageFooter}>
                  <Text style={msg.isSender ? cs.sentTime : cs.receivedTime}>
                    {msg.timestamp || ""}
                  </Text>
                  {msg.isSender && msg.status === "sent" && (
                    <Check color="#ffedd5" size={14} style={cs.tickIcon} />
                  )}
                  {msg.isSender && msg.status === "read" && (
                    <CheckCheck color="#34B7F1" size={15} style={cs.tickIcon} />
                  )}
                </View>
              </Animated.View>
            ))}
          ...
          {isRecordingVoiceNote ? (
            <Animated.View entering={FadeIn} style={cs.recordingContainer}>
              <View style={cs.recordingInfo}>
                ...
                <Text style={cs.recordingText}>
                  Recording {formatDuration(voiceNoteDuration)}
                </Text>
              </View>
              <Pressable
                style={cs.cancelVoiceNoteBtn}
            ...
            <View style={cs.inputContainer}>
              <TextInput
                style={cs.textInput}
                placeholder="अरण्य को संदेश लिखें..."
        ...
        <View style={cms.container}>
          ...
          <View style={cms.viewfinder}>
            {hasCameraPermission && !cameraError ? (
              ...
              <View style={cms.noPermissionContainer}>
                {cameraError ? (
                  <Text style={cms.noPermissionText}>
                    Camera source busy or unavailable
                  </Text>
                ) : (
                  <Text style={cms.noPermissionText}>
          ...
          <View style={cms.galleryContainer}>
            <ScrollView
              ...
              contentContainerStyle={cms.galleryScroll}
            >
          ...
          <View style={cms.shutterRow}>
            ...
            {/* Shutter */}
            <Pressable style={cms.shutterOuter} onPress={takePicture}>
          ...
          <View style={cms.modeRow}>
            <Pressable onPress={() => setCameraMode("video")}>
              <Text
                style={[
                  cms.modeText,
                  cameraMode === "video" && cms.modeTextActive,
                ]}
              >
                Video
              </Text>
            ...
            <View style={cms.modePill}>
              <Text
                style={[
                  cms.modeText,
                  cameraMode === "photo" && cms.modeTextActive,
                ]}
              >
                Photo
              </Text>
            ...
            <Pressable onPress={() => setCameraMode("videonote")}>
              <Text
                style={[
                  cms.modeText,
                  cameraMode === "videonote" && cms.modeTextActive,
                ]}
              >
                Video note
              </Text>
